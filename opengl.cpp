...
vector<int> check_movable_zone(Vehicle a,vector<vector<char>> rd)
{
	...
	for(int i=x_pos+1; i<=x_new_pos; i++)
    {
      ...
      for(int j=y_pos; j>=y_pos-length+1; j--)
	...
	if(lane_change==5)
    {
      vector<int> a{0,5,x_velocity}; //return = new_x_pos, decision
      return a;
    }
	if(lane_change==0)
    {
          ...
          for(int j=y_pos;j>=y_pos-length;j--)
            {
              if(i<rd.size() && i>=0)
                {
                  if(!((i >= x_pos-width+1) && (i<=x_pos) && (j<=y_pos) && (j>=y_pos-length+1)))
                    {
                      if(rd[i][j]!=' ')
                        {
                          move_right=false;
      ...
      while(vehicle_counter>=0)
        {
          ...
          if(start_point+a[vehicle_counter]->get_length()-1>road_length-1)
            {
...
void DrawGLScene()
{
  ...
  glEnd();
	...
	usleep(250000);//sleep for one second
	if(!road_empty)
    {
      road_empty=true;
